...
<div class="space-y-5">
    ...
    <div class="flex flex-wrap md:flex-nowrap gap-3">
        <!-- Total -->
        <div class="flex-1 min-w-[100px] bg-white dark:bg-gray-800 rounded-xl p-3 shadow-sm border border-gray-200 dark:border-gray-700">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {{-- ================= KOLOM KIRI: TIPS CARD (LEBAR NORMAL) ================= --}}
        <div>
            <div class="bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 dark:from-purple-900/30 dark:via-indigo-900/30 dark:to-blue-900/30 rounded-xl shadow-sm border border-purple-200 dark:border-purple-800 overflow-hidden">
                <div class="px-4 py-2.5 border-b border-purple-200 dark:border-purple-800 bg-white/50 dark:bg-gray-800/30">
                ...
                <div class="p-3" x-data="tipsSlider()" x-init="initSlider()">
                    <div class="relative overflow-hidden">
                        <div class="flex transition-transform duration-500 ease-in-out" :style="'transform: translateX(-' + currentIndex * 100 + '%)'">
                            <template x-for="(tip, index) in tips" :key="index">
                                <div class="w-full flex-shrink-0 px-1">
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-purple-100 dark:border-purple-800">
                                        <div class="flex items-start gap-2">
                                            <div class="text-2xl flex-shrink-0" x-text="tip.icon"></div>
                                            <div class="flex-1 min-w-0">
                                                <h4 class="text-sm font-bold text-gray-800 dark:text-white" x-text="tip.title"></h4>
                                                <p class="text-[11px] text-gray-600 dark:text-gray-300 mt-1 leading-relaxed" x-text="tip.content"></p>
                                                <div class="mt-1.5 flex items-center gap-2 flex-wrap">
                                                    <span class="text-[9px] text-purple-500 dark:text-purple-400">💡 Tips #<span x-text="index + 1"></span></span>
                                                    <span class="text-[9px] text-gray-400">•</span>
                                                    <span class="text-[9px] text-gray-400">Bermanfaat untuk pengaduan Anda</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="flex justify-center gap-1 mt-2">
                        <template x-for="(tip, index) in tips" :key="index">
                            <button @click="currentIndex = index" 
                                    ...
                                    :class="currentIndex === index ? 'w-4 bg-purple-500' : 'bg-purple-300 dark:bg-purple-600'">
                            </button>
                        </template>
                    </div>
                    
                    <!-- Prev/Next Buttons -->
                    <button @click="prev()" class="absolute left-0 top-1/2 -translate-y-1/2 -ml-1.5 w-6 h-6 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center hover:bg-purple-50 transition border border-purple-200">
                        <svg class="w-3 h-3 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </button>
                    <button @click="next()" class="absolute right-0 top-1/2 -translate-y-1/2 -mr-1.5 w-6 h-6 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center hover:bg-purple-50 transition border border-purple-200">
                        <svg class="w-3 h-3 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    @if($latestComplaint)
                        <div class="mb-3">
                            ...
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                                <div class="{{ $latestComplaint->getProgressColorAttribute() ?? 'bg-blue-500' }} h-2 rounded-full transition-all duration-500" style="width: {{ $latestComplaint->getProgressPercentageAttribute() ?? 0 }}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2">
                            <p class="text-xs font-medium text-gray-800 dark:text-white truncate">{{ $latestComplaint->judul }}</p>
                            ...
                            @if($latestComplaint->feedbacks->count() > 0)
                                <p class="text-[9px] text-green-600 dark:text-green-400 mt-1 truncate">💬 {{ Str::limit($latestComplaint->feedbacks->last()->pesan, 60) }}</p>
                            @endif
                        </div>
                    @else
