    ...
    <div class="flex flex-col lg:flex-row gap-5">
        ...
        <div class="w-full lg:w-1/2">
            <div class="bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 dark:from-purple-900/30 dark:via-indigo-900/30 dark:to-blue-900/30 rounded-xl shadow-sm border border-purple-200 dark:border-purple-800 overflow-hidden h-full">
                ...
                <div class="p-4" x-data="tipsSlider()" x-init="initSlider()">
                    <div class="relative overflow-hidden">
                        <div class="flex transition-transform duration-500 ease-in-out" :style="'transform: translateX(-' + currentIndex * 100 + '%)'">
                            <template x-for="(tip, index) in tips" :key="index">
                                <div class="w-full flex-shrink-0 px-1">
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm border border-purple-100 dark:border-purple-800">
                                        <div class="flex items-start gap-3">
                                            <div class="text-3xl" x-text="tip.icon"></div>
                                            <div class="flex-1">
                                                <h4 class="text-sm font-bold text-gray-800 dark:text-white" x-text="tip.title"></h4>
                                                <p class="text-xs text-gray-600 dark:text-gray-300 mt-1 leading-relaxed" x-text="tip.content"></p>
                                                <div class="mt-2 flex items-center gap-2">
                                                    <span class="text-[10px] text-purple-500 dark:text-purple-400">💡 Tips #<span x-text="index + 1"></span></span>
                                                    <span class="text-[10px] text-gray-400">•</span>
                                                    <span class="text-[10px] text-gray-400">Bermanfaat untuk pengaduan Anda</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="flex justify-center gap-1.5 mt-3">
                        <template x-for="(tip, index) in tips" :key="index">
                            <button @click="currentIndex = index" 
                                    ...
                                    :class="currentIndex === index ? 'w-5 bg-purple-500' : 'bg-purple-300 dark:bg-purple-600'">
                            </button>
                        </template>
                    </div>
                    
                    <!-- Prev/Next Buttons -->
                    <button @click="prev()" class="absolute left-0 top-1/2 -translate-y-1/2 -ml-2 w-7 h-7 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center hover:bg-purple-50 transition border border-purple-200">
                        <svg class="w-3.5 h-3.5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </button>
                    <button @click="next()" class="absolute right-0 top-1/2 -translate-y-1/2 -mr-2 w-7 h-7 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center hover:bg-purple-50 transition border border-purple-200">
                        <svg class="w-3.5 h-3.5 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
