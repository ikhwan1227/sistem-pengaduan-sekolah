...
@section('content')
<div class="space-y-8 px-3 md:px-6 lg:px-8 max-w-7xl mx-auto">
    
    {{-- ================= STATS CARD ================= --}}
    <div class="flex flex-wrap md:flex-nowrap gap-3">
        <!-- Total -->
        <div class="flex-1 min-w-[100px] bg-white dark:bg-gray-800 rounded-xl p-3 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700">
    ...
    {{-- ================= JARAK BESAR ANTARA STATS CARD DAN KONTEN BAWAH ================= --}}
    <div class="mb-8"></div>

    {{-- ================= LAYOUT 2 KOLOM ================= --}}
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        
        {{-- ================= KOLOM KIRI: TIPS CARD ================= --}}
        <div class="relative z-0">
            <div class="bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 dark:from-indigo-800 dark:via-purple-800 dark:to-pink-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 border border-indigo-300 dark:border-indigo-700 overflow-hidden">
                <!-- Header dengan warna solid -->
                <div class="px-5 py-3 border-b border-white/30 bg-white/10 dark:bg-black/20">
                ...
                <div class="p-5" x-data="tipsSlider()" x-init="initSlider()">
                        ...
                        <div class="flex transition-transform duration-500 ease-out" :style="'transform: translateX(-' + currentIndex * 100 + '%)'">
                            <template x-for="(tip, index) in tips" :key="index">
                                <div class="w-full flex-shrink-0 px-1">
                                    <div class="bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm rounded-xl p-5 shadow-lg border border-white/30">
                    ...
                    <!-- Dots Navigation dengan warna cerah -->
                    <div class="flex justify-center gap-3 mt-5">
                        <template x-for="(tip, index) in tips" :key="index">
                            <button @click="currentIndex = index" 
                                    class="rounded-full transition-all duration-300"
                                    :class="currentIndex === index ? 'w-8 h-2 bg-yellow-400 shadow-md' : 'w-2 h-2 bg-white/50'">
                            </button>
                        </template>
                    </div>
                    
                    <!-- Tombol Prev/Next -->
                    <button @click="prev()" class="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80 dark:bg-gray-800/80 shadow-md flex items-center justify-center hover:bg-white transition-all duration-300 border border-gray-300">
                        <svg class="w-4 h-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </button>
                    <button @click="next()" class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80 dark:bg-gray-800/80 shadow-md flex items-center justify-center hover:bg-white transition-all duration-300 border border-gray-300">
                        <svg class="w-4 h-4 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="px-5 py-3 border-t border-white/30 bg-black/10 dark:bg-black/20">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
                            <span class="text-[10px] text-white/80">✨ Tips berganti otomatis setiap 5 detik</span>
                        </div>
                        <div class="flex items-center gap-1">
                            <span class="text-[10px] text-white/60">⏱️</span>
                            <span class="text-[10px] text-white/60">Tips #<span x-text="currentIndex + 1"></span>/<span x-text="tips.length"></span></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col gap-6">
            ...
            {{-- QUICK ACTION --}}
            <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 dark:from-indigo-700 dark:to-indigo-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden">
                <div class="px-5 py-3 bg-white/10 dark:bg-black/20">
                ...
                <div class="p-4">
                    <div class="flex gap-3">
                        <a href="{{ route('complaint.index') }}" class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-white text-indigo-600 font-semibold text-sm rounded-lg transition-all duration-200 shadow-md hover:shadow-lg hover:bg-gray-100">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                            </svg>
                            Buat
                        </a>
                        <a href="{{ route('complaint.history') }}" class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-white/20 hover:bg-white/30 text-white text-sm rounded-lg transition-all duration-200">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                            </svg>
                            Histori
                        </a>
                        <a href="{{ route('student.tips') }}" class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-white/20 hover:bg-white/30 text-white text-sm rounded-lg transition-all duration-200">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Tips
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30">
                    ...
                    @if($latestComplaint)
                        <div class="mb-4">
                            ...
                            <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                                <div class="h-2 rounded-full transition-all duration-500 bg-gradient-to-r from-indigo-500 to-purple-500" style="width: {{ $latestComplaint->getProgressPercentageAttribute() ?? 0 }}%;"></div>
                            </div>
                        </div>
                    ...
                    @forelse($pengaduanTerbaru->take(3) as $pengaduan)
                        <div class="p-3 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition duration-200">
                            <div class="flex justify-between items-start">
                                ...
                                <a href="{{ route('complaint.history') }}" class="text-indigo-500 text-[11px] ml-2 shrink-0 hover:underline">Detail</a>
                            </div>
                        </div>
                    @empty
