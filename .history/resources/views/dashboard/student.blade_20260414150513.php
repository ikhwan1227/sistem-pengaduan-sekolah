...
<div class="dashboard-col" x-data="tipsSlider()" x-init="initSlider()">
    <div class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-indigo-950/30 dark:via-purple-950/30 dark:to-pink-950/30 rounded-xl shadow-[0_8px_25px_rgba(37,99,235,0.2)] hover:shadow-[0_12px_35px_rgba(37,99,235,0.3)] transition-all duration-300 border border-indigo-200 dark:border-indigo-800 overflow-hidden flex flex-col h-full">
        ...
        <div class="flex-1 p-5 relative">
                ...
                <div class="flex transition-transform duration-500 ease-out" :style="'transform: translateX(-' + currentIndex * 100 + '%)'">
                    <template x-for="(tip, index) in tips" :key="index">
                        <div class="w-full flex-shrink-0">
                            <div class="rounded-xl p-6 shadow-lg border-2" :class="getCardClass(tip.color)">
                                <div class="text-center mb-4">
                                    <span class="text-6xl" x-text="tip.icon"></span>
                                </div>
                                <h4 class="text-xl font-bold text-center mb-3" :class="getTitleClass(tip.color)" x-text="tip.title"></h4>
                                <p class="text-sm text-center leading-relaxed" :class="getTextClass(tip.color)" x-text="tip.content"></p>
                                <div class="mt-5 flex justify-center">
                                    <span class="text-[11px] px-3 py-1.5 rounded-full font-medium" :class="getBadgeClass(tip.color)">
            ...
            {{-- Tombol Prev/Next --}}
            <button @click="prev()" class="absolute left-0 top-1/2 -translate-y-1/2 -ml-3 w-8 h-8 rounded-full bg-white dark:bg-gray-800 shadow-lg flex items-center justify-center hover:scale-110 transition-all duration-300 border border-gray-200 dark:border-gray-600 z-10">
                <svg class="w-4 h-4 text-gray-600 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
            </button>
            <button @click="next()" class="absolute right-0 top-1/2 -translate-y-1/2 -mr-3 w-8 h-8 rounded-full bg-white dark:bg-gray-800 shadow-lg flex items-center justify-center hover:scale-110 transition-all duration-300 border border-gray-200 dark:border-gray-600 z-10">
                <svg class="w-4 h-4 text-gray-600 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex justify-center items-center gap-2">
                <template x-for="(tip, index) in tips" :key="index">
                    <button @click="currentIndex = index" 
                            ...
                            :class="currentIndex === index ? 'w-6 bg-[#2563eb] shadow-sm' : 'w-2 bg-gray-400 dark:bg-gray-500 hover:bg-gray-500'">
                    </button>
                </template>
            </div>
            <div class="text-center mt-2">
                <span class="text-[10px] text-gray-500 dark:text-gray-400">Tips <span x-text="currentIndex + 1"></span> dari <span x-text="tips.length"></span></span>
            </div>
        </div>
    </div>
</div>
        ...
        return {
            ...
            getCardClass(color) {
                const classes = {
                    ...
                    orange: 'bg-orange-50 dark:bg-orange-950/40 border-orange-300 dark:border-orange-700',
                    pink: 'bg-pink-50 dark:bg-pink-950/40 border-pink-300 dark:border-pink-700',
                    teal: 'bg-teal-50 dark:bg-teal-950/40 border-teal-300 dark:border-teal-700',
                    indigo: 'bg-indigo-50 dark:bg-indigo-950/40 border-indigo-300 dark:border-indigo-700',
                    red: 'bg-red-50 dark:bg-red-950/40 border-red-300 dark:border-red-700'
                };
            ...
            getBadgeClass(color) {
                const classes = {
                    blue: 'bg-blue-200 text-blue-800 dark:bg-blue-900/70 dark:text-blue-200',
                    green: 'bg-green-200 text-green-800 dark:bg-green-900/70 dark:text-green-200',
                    purple: 'bg-purple-200 text-purple-800 dark:bg-purple-900/70 dark:text-purple-200',
                    orange: 'bg-orange-200 text-orange-800 dark:bg-orange-900/70 dark:text-orange-200',
                    pink: 'bg-pink-200 text-pink-800 dark:bg-pink-900/70 dark:text-pink-200',
                    teal: 'bg-teal-200 text-teal-800 dark:bg-teal-900/70 dark:text-teal-200',
                    indigo: 'bg-indigo-200 text-indigo-800 dark:bg-indigo-900/70 dark:text-indigo-200',
                    red: 'bg-red-200 text-red-800 dark:bg-red-900/70 dark:text-red-200'
                };
                return classes[color] || classes.blue;
            }
        }
