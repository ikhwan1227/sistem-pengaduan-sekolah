...
<div>
    <div class="rounded-xl shadow-lg overflow-hidden"
        ...
        <!-- Header -->
        <div class="px-5 py-4 border-b border-white/30">
                ...
                <div class="flex transition-transform duration-500 ease-out" :style="'transform: translateX(-' + currentIndex * 100 + '%)'">
                    <template x-for="(tip, index) in tips" :key="index">
                        <div class="w-full flex-shrink-0 px-1">
                            <div class="bg-white/15 backdrop-blur-sm rounded-xl p-5 border border-white/30 shadow-lg">
                                <div class="flex items-start gap-4">
                                    <div class="text-5xl" x-text="tip.icon"></div>
                                    <div class="flex-1">
                                        <h4 class="text-lg font-bold text-white" x-text="tip.title"></h4>
                                        <p class="text-sm text-white/90 mt-2 leading-relaxed" x-text="tip.content"></p>
                                        <div class="mt-3 flex items-center gap-2">
                                            <span class="text-[10px] px-2 py-1 rounded-full bg-white/30 text-white font-semibold">💡 Tips #<span x-text="index + 1"></span></span>
                                            <span class="text-[10px] text-white/60">•</span>
                                            <span class="text-[10px] text-white/60">Bermanfaat untuk Anda</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="flex justify-center gap-2 mt-5">
                <template x-for="(tip, index) in tips" :key="index">
                    <button @click="currentIndex = index" 
                            ...
                            :class="currentIndex === index ? 'w-7 bg-white' : 'w-2 bg-white/40'">
                    </button>
                </template>
            </div>
            
            <!-- Tombol Prev/Next -->
            <button @click="prev()" class="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/30 backdrop-blur-sm flex items-center justify-center hover:bg-white/50 transition-all duration-300 shadow-lg">
                <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                </svg>
            </button>
            <button @click="next()" class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/30 backdrop-blur-sm flex items-center justify-center hover:bg-white/50 transition-all duration-300 shadow-lg">
                <svg class="w-4 h-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <!-- Footer -->
        <div class="px-5 py-3 border-t border-white/30 bg-black/10">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
                    <span class="text-[10px] text-white/70">✨ Tips berganti otomatis setiap 5 detik</span>
                </div>
                <div class="flex items-center gap-1">
                    <span class="text-[10px] text-white/50">⏱️</span>
                    <span class="text-[10px] text-white/50">Tips #<span x-text="currentIndex + 1"></span>/<span x-text="tips.length"></span></span>
                </div>
            </div>
        </div>
    </div>
                    ...
                    @if($latestComplaint)
                        <div class="mb-4">
                            <div class="flex justify-between text-xs text-gray-600 dark:text-gray-400 mb-1">
                                <span>Progress</span>
                                <span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ $latestComplaint->getProgressPercentageAttribute() ?? 0 }}%</span>
                            </div>
