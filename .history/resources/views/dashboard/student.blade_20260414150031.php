...
<div class="dashboard-col">
    <div class="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-indigo-950/30 dark:via-purple-950/30 dark:to-pink-950/30 rounded-xl shadow-[0_8px_25px_rgba(37,99,235,0.2)] hover:shadow-[0_12px_35px_rgba(37,99,235,0.3)] transition-all duration-300 border border-indigo-200 dark:border-indigo-800 overflow-hidden flex flex-col h-full">
        ...
        {{-- Content Slider --}}
        <div class="flex-1 p-5" x-data="tipsSlider()" x-init="initSlider()">
            <div class="relative">
                    ...
                    <div class="flex transition-transform duration-500 ease-out" :style="'transform: translateX(-' + currentIndex * 100 + '%)'">
                        <template x-for="(tip, index) in tips" :key="index">
                            <div class="w-full flex-shrink-0">
                                <div class="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-md border" :class="getBorderClass(index)">
                                    <div class="text-center mb-3">
                                        <span class="text-5xl" x-text="tip.icon"></span>
                                    </div>
                                    <h4 class="text-lg font-bold text-center mb-3" :class="getTitleClass(index)" x-text="tip.title"></h4>
                                    <p class="text-sm text-gray-600 dark:text-gray-300 text-center leading-relaxed" x-text="tip.content"></p>
                                    <div class="mt-4 flex justify-center">
                                        <span class="text-[11px] px-3 py-1 rounded-full" :class="getBadgeClass(index)">
                ...
                {{-- Tombol Prev/Next --}}
                <button @click="prev()" class="absolute left-0 top-1/2 -translate-y-1/2 -ml-3 w-7 h-7 rounded-full bg-white dark:bg-gray-800 shadow-md flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-300 border border-gray-200 dark:border-gray-600 z-10">
                    <svg class="w-3 h-3 text-gray-600 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </button>
                <button @click="next()" class="absolute right-0 top-1/2 -translate-y-1/2 -mr-3 w-7 h-7 rounded-full bg-white dark:bg-gray-800 shadow-md flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-300 border border-gray-200 dark:border-gray-600 z-10">
                    <svg class="w-3 h-3 text-gray-600 dark:text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex justify-center items-center gap-3">
                <template x-for="(tip, index) in tips" :key="index">
                    <button @click="currentIndex = index" 
                            ...
                            :class="currentIndex === index ? 'w-6 bg-[#2563eb]' : 'w-2 bg-gray-300 dark:bg-gray-600 hover:bg-gray-400'">
                    </button>
                </template>
            </div>
            <div class="text-center mt-2">
                <span class="text-[10px] text-gray-400 dark:text-gray-500">Tips #<span x-text="currentIndex + 1"></span> dari <span x-text="tips.length"></span></span>
            </div>
        </div>
    </div>
</div>
