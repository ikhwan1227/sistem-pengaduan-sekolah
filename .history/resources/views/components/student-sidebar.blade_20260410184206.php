...
<div x-data="{ 
    ...
    sidebarHover: false,
    get sidebarOpen() {
        return this.sidebarLocked || this.sidebarHover
    },
    toggleLock() {
     ...
     class="bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-200 flex flex-col flex-shrink-0 h-screen sticky top-0">
    
    <!-- Menu Items dengan Tombol Lock di samping Dashboard -->
    <nav class="flex-1 mt-4 px-2 space-y-1 overflow-y-auto">
        @foreach($menuItems as $index => $item)
            <div class="flex items-center">
                <a href="{{ $item['route'] == '#' ? '#' : route($item['route']) }}" 
                   class="flex-1 flex items-center px-3 py-2.5 rounded-lg transition-colors duration-200 group
                          {{ $item['active'] 
                    ...
                    <span x-show="sidebarOpen" class="ml-3 text-sm font-medium whitespace-nowrap" x-cloak>{{ $item['name'] }}</span>
                </a>
                
                <!-- Tombol Lock/Unlock - Hanya di samping menu Dashboard (index ke-0) -->
                @if($index === 0)
                    <button @click="toggleLock()" 
                            ...
                            class="ml-1 w-7 h-7 rounded-lg flex items-center justify-center transition-colors hover:bg-gray-200 dark:hover:bg-gray-600 flex-shrink-0"
                            x-show="sidebarOpen"
                            x-cloak>
    ...
    <!-- Tips Ringkas -->
    <div x-show="sidebarOpen" class="mx-2 mb-2 p-2.5 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex-shrink-0" x-cloak>
        ...
        <p class="text-xs text-gray-600 dark:text-gray-400 mt-1 leading-tight">
            Deskripsikan masalah dengan detail agar cepat ditindaklanjuti.
        </p>
    </div>
    
    <!-- User Info Bottom -->
    <div class="p-2.5 border-t border-gray-200 dark:border-gray-700 flex-shrink-0">
        <div x-show="sidebarOpen" class="flex items-center space-x-2" x-cloak>
            <div class="w-8 h-8 rounded-full bg-gradient-to-r from-green-400 to-blue-500 flex items-center justify-center text-white text-xs font-semibold">
