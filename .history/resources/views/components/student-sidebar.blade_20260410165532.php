...
<div id="sidebar" class="w-48 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-200 flex flex-col flex-shrink-0 min-h-screen sticky top-0">
    ...
    <div class="h-14 flex items-center justify-between px-2 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <div class="flex items-center space-x-1.5">
            ...
            <span class="sidebar-text font-bold text-gray-800 dark:text-white text-xs">Siswa Panel</span>
        </div>
        
        <!-- Tombol Lock/Unlock -->
        <button id="sidebarLockBtn" class="w-5 h-5 rounded-full flex items-center justify-center transition-colors hover:bg-gray-200 dark:hover:bg-gray-600 bg-gray-100 dark:bg-gray-700 text-gray-500">
        ...
        @foreach($menuItems as $item)
            <a href="{{ $item['route'] == '#' ? '#' : route($item['route']) }}" 
               class="flex items-center px-2 py-1.5 rounded-lg transition-colors duration-200 group
                ...
                <span class="sidebar-text ml-2.5 text-xs font-medium whitespace-nowrap">{{ $item['name'] }}</span>
            </a>
        @endforeach
    </nav>
    
    <!-- Tips Ringkas -->
    <div class="mx-1.5 mb-1.5 p-1.5 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex-shrink-0">
