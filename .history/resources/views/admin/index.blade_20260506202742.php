    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 mb-6 overflow-hidden">
        ...
        <div class="p-5">
            <form method="GET" class="flex flex-wrap gap-3 items-end">
                ...
                <div class="flex-1 min-w-[150px]">
                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Siswa</label>
                    <select name="user_id" class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white p-2 rounded-lg text-sm focus:ring-2 focus:ring-blue-500">
    ...
    @if(isset($complaints) && $complaints->count() > 0)
        <div class="space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-all duration-200">
                {{-- Header Card --}}
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700/30 dark:to-gray-800/30 px-5 py-3 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex flex-wrap justify-between items-center gap-2">
                        <div class="flex items-center gap-2">
                            <div class="w-2 h-2 rounded-full bg-blue-500"></div>
                            <h2 class="font-semibold text-gray-800 dark:text-white text-base">{{ $c->judul }}</h2>
                        </div>
                ...
                {{-- Body Card --}}
                <div class="p-5">
                    <p class="text-gray-600 dark:text-gray-300 text-sm mb-3 leading-relaxed">{{ $c->deskripsi }}</p>
                    
                    @if($c->image)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $c->image) }}" class="w-32 h-24 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                        </div>
                    @endif
                    
                    {{-- Informasi Detail --}}
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm mb-4 p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                            </svg>
                            <span class="text-gray-500 dark:text-gray-400">Siswa:</span>
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $c->user->name ?? '-' }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $c->category->nama_kategori ?? '-' }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            ...
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ \Carbon\Carbon::parse($c->tanggal)->translatedFormat('d F Y') }}</span>
                        </div>
                        @if($c->location)
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $c->location }}</span>
                        </div>
                        @endif
                    </div>
                    ...
                    @if($c->feedbacks->count() > 0)
                        <div class="mb-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-3 border-l-4 border-emerald-500">
                            <div class="flex items-center gap-2 mb-1">
                                <svg class="w-4 h-4 text-emerald-600 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                </svg>
                                <p class="text-xs font-semibold text-emerald-700 dark:text-emerald-400">Feedback Admin</p>
                            </div>
                            <p class="text-sm text-emerald-700 dark:text-emerald-300">{{ $c->feedbacks->last()->pesan }}</p>
                            <p class="text-[10px] text-emerald-500 dark:text-emerald-500 mt-1">{{ \Carbon\Carbon::parse($c->feedbacks->last()->tanggal)->translatedFormat('d F Y H:i') }}</p>
                        </div>
                    @endif
                    ...
                    <div class="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                        <form method="POST" action="/admin/status/{{ $c->id }}" class="flex gap-2">
                            @csrf
                            <select name="status" class="border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg p-1.5 text-sm focus:ring-2 focus:ring-blue-500">
                            ...
                            <button class="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1.5 rounded-lg text-sm transition shadow-sm">
                                Update
                            </button>
                        </form>
                        ...
                        <form method="POST" action="/admin/feedback/{{ $c->id }}" class="flex gap-2">
                            @csrf
                            <input type="text" name="pesan" placeholder="Tulis feedback..." class="border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg p-1.5 text-sm w-48 focus:ring-2 focus:ring-blue-500">
                            <button class="bg-purple-500 hover:bg-purple-600 text-white px-3 py-1.5 rounded-lg text-sm transition shadow-sm">
                                Kirim
                            </button>
                        </form>
                        ...
                        <form method="POST" action="/admin/delete/{{ $c->id }}" onsubmit="return confirm('Yakin ingin menghapus pengaduan ini?')">
                            @csrf
                            @method('DELETE')
                            <button class="bg-rose-500 hover:bg-rose-600 text-white px-3 py-1.5 rounded-lg text-sm transition shadow-sm">
                                🗑️ Hapus
                            </button>
                        </form>
                    </div>
