...
<div x-show="activeTab === 'histori'" x-cloak>
    <div class="space-y-4">
        @forelse($complaints as $c)
            <article class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-all duration-200">
                {{-- Header Card --}}
                ...
                {{-- Body Card --}}
                <div class="px-5 py-4">
                    <p class="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">{{ $c->deskripsi }}</p>
                    
                    {{-- Gambar jika ada --}}
                    @if($c->image)
                        <div class="mt-3">
                            <img src="{{ asset('storage/' . $c->image) }}" class="w-32 h-32 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-600">
                        </div>
                    @endif
                </div>
                ...
                @if($c->feedbacks->count() > 0)
                    <div class="mx-5 mb-4 bg-green-50 dark:bg-green-900/20 rounded-lg p-4 border-l-4 border-green-500">
                        <div class="flex items-start gap-2">
                            <div class="w-6 h-6 rounded-full bg-green-100 dark:bg-green-800/50 flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1">
                                ...
                                @foreach($c->feedbacks as $f)
                                    ...
                                    <p class="text-xs text-green-500/70 dark:text-green-400/70 mt-1">
                                        {{ \Carbon\Carbon::parse($f->tanggal)->translatedFormat('d F Y H:i') }}
                                    </p>
                                @endforeach
                            </div>
                        </div>
