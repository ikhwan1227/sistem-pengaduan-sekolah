...
<div class="space-y-6 px-4 md:px-6 lg:px-8" x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'buat' }" x-init="$watch('activeTab', value => window.location.hash = value)">
    ...
    {{-- ================= KONTEN TAB HISTORI ================= --}}
    {{-- KOLOM KANAN: History Pengaduan (Horizontal Scroll) --}}
<section class="dashboard-col equal-height-wrapper">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-[0_8px_25px_rgba(37,99,235,0.15)] hover:shadow-[0_12px_35px_rgba(37,99,235,0.25)] transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden equal-height-content">
        ...
        <div class="history-horizontal" style="display: flex; overflow-x: auto; gap: 1rem; padding: 1rem; flex: 1;">
            @forelse($pengaduanTerbaru->take(5) as $pengaduan)
                <article class="history-card" style="flex: 0 0 280px; background: white; border-radius: 12px; border: 1px solid #e5e7eb; overflow: hidden; transition: all 0.3s ease;">
                    {{-- Header Card --}}
                    <div class="p-3 border-b border-gray-100 dark:border-gray-700" style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);">
                        <div class="flex items-center justify-between mb-2">
                            <h3 class="font-semibold text-sm text-gray-800 dark:text-white truncate" style="max-width: 160px;">{{ $pengaduan->judul }}</h3>
                            <span class="status-badge text-[10px] px-2 py-0.5 rounded-full font-medium
                                ...
                                @endif">
                                @if($pengaduan->status == 'pending') ⏳ Pending
                                @elseif($pengaduan->status == 'diproses') 🔄 Diproses
                                @elseif($pengaduan->status == 'selesai') ✅ Selesai
                                @else ❌ Ditolak
                                @endif
                            </span>
                        </div>
                        <div class="flex items-center gap-2 text-xs text-gray-500">
                            <span>📂 {{ $pengaduan->category->nama_kategori ?? 'Umum' }}</span>
                            <span>•</span>
                    ...
                    <div class="p-3 min-h-[100px]">
                        <p class="text-xs text-gray-600 dark:text-gray-300 line-clamp-3">
                            {{ Str::limit($pengaduan->isi_pengaduan ?? $pengaduan->deskripsi ?? 'Tidak ada deskripsi', 100) }}
                        </p>
                        ...
                        @if($pengaduan->feedbacks->count() > 0)
                            <div class="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                                <div class="flex items-start gap-1.5">
                                    ...
                                    <p class="text-[11px] text-emerald-600 dark:text-emerald-400 line-clamp-2">
                                        <span class="font-medium">Admin:</span> {{ Str::limit($pengaduan->feedbacks->last()->pesan, 60) }}
                                    </p>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="p-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-1">
                                <span class="text-[10px] text-gray-400">ID: #{{ $pengaduan->id }}</span>
                            </div>
                            <a href="{{ route('complaint.show', $pengaduan->id) }}" 
                               class="text-indigo-600 dark:text-indigo-400 text-xs font-medium hover:underline flex items-center gap-1">
                                Detail
                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @empty
                <div class="w-full text-center py-8 text-gray-500 dark:text-gray-400">
                    ...
                    <a href="{{ route('complaint.index') }}" class="inline-block mt-2 text-xs text-indigo-500 hover:underline">Buat pengaduan pertama</a>
                </div>
            @endforelse
        </div>
    </div>
