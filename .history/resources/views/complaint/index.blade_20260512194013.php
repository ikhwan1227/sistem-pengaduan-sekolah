...
<div class="space-y-6 px-4 md:px-6 lg:px-8" x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'buat' }" x-init="$watch('activeTab', value => window.location.hash = value)">
                ...
                <div class="p-6 md:p-8">
                    <form method="POST" action="{{ isset($draft) ? route('complaint.store') : route('complaint.store') }}" enctype="multipart/form-data" id="complaintForm">
                        ...
                        <div class="flex flex-col sm:flex-row justify-end gap-3 mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <button type="submit" class="px-6 py-2.5 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-medium rounded-xl transition-all duration-200 flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Dibuat</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @if(isset($drafts) && $drafts->count() > 0)
                            @foreach($drafts as $index => $draftItem)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                                ...
                                <td class="px-4 py-3">
                                    <div class="text-sm font-medium text-gray-800 dark:text-white">{{ $draftItem->judul }}</div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mt-1">
                                        {{ Str::limit($draftItem->deskripsi, 60) }}
                                    </div>
                                    <span class="inline-block mt-1 px-1.5 py-0.5 text-[9px] font-semibold rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
                                        Draft
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                                        {{ $draftItem->category->nama_kategori ?? '-' }}
                                    </span>
                                </td>
                                
                                {{-- Lokasi --}}
                                <td class="px-4 py-3">
                                    <div class="text-sm text-gray-600 dark:text-gray-400 max-w-[150px] truncate" title="{{ $draftItem->location }}">
                                        {{ $draftItem->location ?? '-' }}
                                    </div>
                                </td>
                                ...
                                {{-- Tanggal dibuat --}}
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    {{ \Carbon\Carbon::parse($draftItem->tanggal)->format('d/m/Y H:i') }}
                                </td>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Gambar</th>
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Feedback Admin</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 mt-1">{{ Str::limit($c->deskripsi, 60) }}</div>
                            </td>
                            ...
                            {{-- Lokasi --}}
                            <td class="px-4 py-3">
                                <div class="text-sm text-gray-600 dark:text-gray-400 max-w-[150px] truncate" title="{{ $c->location }}">
                                    {{ $c->location ?? '-' }}
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                @if($c->image)
                                    <button onclick="showImageModal('{{ asset('storage/' . $c->image) }}', '{{ $c->judul }}')" 
                                            class="focus:outline-none">
                                        <img src="{{ asset('storage/' . $c->image) }}" 
                                            class="w-12 h-12 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 hover:opacity-80 transition cursor-pointer"
                            ...
                            <td class="px-4 py-3">
                                @if($c->feedbacks->count() > 0)
                                    @php
                                        ...
                                        $hasMore = $feedbacks->count() > 1;
                                    @endphp
                                    
                                    <div x-data="{ showAll: false }" class="max-w-[250px]">
                                        {{-- Header dengan tombol lihat semua (jika ada lebih dari 1 feedback) --}}
                                        @if($hasMore)
                                            <div class="flex justify-end mb-1">
                                                <button @click="showAll = !showAll" 
                                                        class="text-[10px] font-medium px-2 py-0.5 rounded-full transition-all duration-200
                                                            bg-purple-100 text-purple-600 hover:bg-purple-200 
                                                            dark:bg-purple-900/30 dark:text-purple-400 dark:hover:bg-purple-900/50">
                                                    <span x-show="!showAll">📋 Lihat Semua ({{ $feedbacks->count() }})</span>
                                                    <span x-show="showAll">⬆️ Sembunyikan</span>
                                                </button>
                                            </div>
                                        @endif
                                        
                                        {{-- Tampilkan feedback terbaru (default) --}}
                                        <div x-show="!showAll" class="bg-gray-50 dark:bg-gray-700/30 rounded-md p-2 border-l-2 border-purple-400">
                                            <div class="flex items-center justify-between mb-0.5">
                                                <span class="text-[10px] font-semibold text-purple-600 dark:text-purple-400">Admin</span>
                                                <span class="text-[9px] text-gray-400">{{ \Carbon\Carbon::parse($latestFeedback->tanggal)->format('d/m/Y H:i') }}</span>
                                            </div>
                                            <p class="text-xs text-gray-700 dark:text-gray-300 line-clamp-2">{{ $latestFeedback->pesan }}</p>
                                        </div>
                                        
                                        {{-- Tampilkan semua feedback (saat tombol diklik) --}}
                                        <div x-show="showAll" x-cloak class="space-y-2 max-h-48 overflow-y-auto pr-1">
                                            @foreach($feedbacks->reverse() as $fb)
                                                <div class="bg-gray-50 dark:bg-gray-700/30 rounded-md p-2 border-l-2 border-purple-400">
                                                    <div class="flex items-center justify-between mb-0.5">
                                                        <span class="text-[10px] font-semibold text-purple-600 dark:text-purple-400">Admin</span>
                                                        <span class="text-[9px] text-gray-400">{{ \Carbon\Carbon::parse($fb->tanggal)->format('d/m/Y H:i') }}</span>
                                                    </div>
                                                    <p class="text-xs text-gray-700 dark:text-gray-300">{{ $fb->pesan }}</p>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                @else
                                    <span class="text-xs text-gray-400 italic">Belum ada balasan</span>
                                @endif
                            </td>
                        </tr>
                        @empty
    ...
    {{-- ================= MODAL ZOOM GAMBAR ================= --}}
    <div id="imageModal" class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 hidden" onclick="closeImageModal()">
        <div class="relative max-w-4xl max-h-[90vh] p-4" onclick="event.stopPropagation()">
            ...
            <img id="modalImage" src="" alt="Bukti Pengaduan" class="max-w-full max-h-[85vh] rounded-lg shadow-2xl">
            <p id="modalImageCaption" class="text-center text-white text-sm mt-3"></p>
        </div>
    </div>
</div>
...
<script>
    ...
    function showImageModal(imageUrl, judul) {
