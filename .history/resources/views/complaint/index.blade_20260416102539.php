    ...
    <div class="border-b border-gray-200 dark:border-gray-700">
        <nav class="flex space-x-4 md:space-x-8">
            ...
            <button @click="activeTab = 'buat'" 
                    :class="activeTab === 'buat' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                    class="py-3 px-1 border-b-2 font-medium text-sm transition-all duration-200 flex items-center gap-2">
            ...
            <button @click="activeTab = 'histori'" 
                    :class="activeTab === 'histori' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                    class="py-3 px-1 border-b-2 font-medium text-sm transition-all duration-200 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if($complaints->total() > 0)
                    <span class="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs px-2 py-0.5 rounded-full">{{ $complaints->total() }}</span>
                @endif
            </button>
            
            {{-- Tab Draft --}}
            <button @click="activeTab = 'draft'" 
                    :class="activeTab === 'draft' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'"
                    class="py-3 px-1 border-b-2 font-medium text-sm transition-all duration-200 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if(isset($drafts) && $drafts->count() > 0)
                    <span class="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400 text-xs px-2 py-0.5 rounded-full">{{ $drafts->count() }}</span>
                @endif
            </button>
        </nav>
    </div>
    ...
    <div class="flex justify-center">
        <div class="w-full max-w-4xl">
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                {{-- KOLOM KIRI: Form Pengaduan (2/3 lebar) --}}
                <div class="flex-[2]">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-[0_8px_25px_rgba(37,99,235,0.15)] hover:shadow-[0_12px_35px_rgba(37,99,235,0.25)] transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden">
                        ...
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 card-header-gradient">
                            <div>
                                ...
                                <p class="text-xs text-white/80 mt-0.5">Buat laporan masalah fasilitas sekolah</p>
                            </div>
                        </div>
                        
                        {{-- Form dengan tinggi terbatas dan scrollbar --}}
                            ...
                            <form method="POST" action="{{ isset($draft) ? route('complaint.store') : route('complaint.store') }}" enctype="multipart/form-data" id="complaintForm">
                                ...
                                <div class="mb-6">
                                    ...
                                    <div x-data="fileUpload()" x-init="init()" class="w-full">
                                        <label for="image-upload" 
                                               class="flex flex-col items-center justify-center w-full h-28 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-xl cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-all duration-200"
                                               :class="isDragging ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : ''"
                                               @dragover.prevent="isDragging = true"
                                               @dragleave.prevent="isDragging = false"
                                               @drop.prevent="handleDrop($event)">
                                            
                                            <div class="flex flex-col items-center justify-center pt-4 pb-4" x-show="!filePreview && !fileName">
                                                <svg class="w-7 h-7 mb-1 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            <div x-show="filePreview" class="relative w-full h-full flex flex-col items-center justify-center">
                                                <img :src="filePreview" class="max-h-20 rounded-lg shadow-sm object-contain">
                                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1" x-text="fileName"></p>
                                                <button type="button" @click="removeFile()" class="absolute top-0 right-0 mt-1 mr-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition">
                                            ...
                                            <div x-show="fileName && !filePreview" class="flex flex-col items-center justify-center">
                                                <svg class="w-7 h-7 mb-1 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                </svg>
                                                <p class="text-xs text-gray-700 dark:text-gray-300" x-text="fileName"></p>
                                                <button type="button" @click="removeFile()" class="mt-1 text-xs text-red-500 hover:text-red-600">Hapus</button>
                                            </div>
                                ...
                                <div class="flex flex-col sm:flex-row justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                    <button type="submit" name="action" value="draft" formaction="{{ route('complaint.draft') }}" 
                                            class="px-5 py-2 bg-gray-500 hover:bg-gray-600 text-white font-medium rounded-lg shadow-sm transition-all duration-200 flex items-center justify-center gap-2">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                                        </svg>
                                        Simpan ke Draft
                                    </button>
                                    <button type="submit" class="px-5 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-medium rounded-lg shadow-sm transition-all duration-200 flex items-center justify-center gap-2">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                {{-- KOLOM KANAN: Card Informasi Pengaduan (1/3 lebar) --}}
                <div class="flex-[1]">
                    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 rounded-xl border border-blue-200 dark:border-blue-800 overflow-hidden sticky top-24">
                        <div class="px-5 py-4 border-b border-blue-200 dark:border-blue-800 bg-white/50 dark:bg-gray-800/30">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-gray-600 dark:text-gray-300">
                                        <span class="font-semibold text-blue-600 dark:text-blue-400">📝 Tips:</span> 
                                        Sebelum mengisi form, usahakan Anda mengikuti tips yang sudah disediakan pada halaman dashboard.
                                    </p>
                                </div>
                            </div>
                            
                            <div class="flex items-start gap-3">
                                <div class="w-7 h-7 rounded-full bg-green-100 dark:bg-green-900/50 flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-gray-600 dark:text-gray-300">
                                        ...
                                        Anda bisa melihat progres pengaduan melalui bar progres ataupun status pengaduan di halaman dashboard.
                                    </p>
                                </div>
                            </div>
                            
                            <div class="flex items-start gap-3">
                                <div class="w-7 h-7 rounded-full bg-purple-100 dark:bg-purple-900/50 flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-gray-600 dark:text-gray-300">
                                        <span class="font-semibold text-purple-600 dark:text-purple-400">📋 Histori:</span> 
                                        Pengaduan yang telah terkirim akan disimpan di histori pengaduan dan dapat Anda pantau kapan saja.
                                    </p>
                                </div>
                            </div>
                            
                            <div class="flex items-start gap-3">
                                <div class="w-7 h-7 rounded-full bg-yellow-100 dark:bg-yellow-900/50 flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-gray-600 dark:text-gray-300">
                                        <span class="font-semibold text-yellow-600 dark:text-yellow-400">💾 Draft:</span> 
                                        Pengaduan yang belum dikirim (draft) akan disimpan di draft pengaduan dan dapat diedit kapan saja.
                                    </p>
                                </div>
                            </div>
                        </div>
