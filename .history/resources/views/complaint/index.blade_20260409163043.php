...
<div class="max-w-4xl mx-auto mt-10 px-4">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        <form method="POST" action="/complaint" enctype="multipart/form-data">
            ...
            <input type="text" name="judul" placeholder="Judul" class="w-full border p-2 mb-1 @error('judul') border-red-500 @enderror" value="{{ old('judul') }}" required>
            @error('judul')
                <p class="text-red-500 text-sm mb-3">{{ $message }}</p>
            @enderror

            <select name="category_id" class="w-full border p-2 mb-1 @error('category_id') border-red-500 @enderror" required>
            ...
            <textarea name="deskripsi" placeholder="Deskripsi" class="w-full border p-2 mb-1 @error('deskripsi') border-red-500 @enderror" required>{{ old('deskripsi') }}</textarea>
            @error('deskripsi')
                <p class="text-red-500 text-sm mb-3">{{ $message }}</p>
            @enderror

            <input type="file" name="image" accept="image/*" class="w-full border p-2 mb-1 @error('image') border-red-500 @enderror">
            @error('image')
                <p class="text-red-500 text-sm mb-3">{{ $message }}</p>
            @enderror
