...
<div class="space-y-6 px-4 md:px-6 lg:px-8" x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'buat' }" x-init="$watch('activeTab', value => window.location.hash = value)">
    ...
    <div x-show="activeTab === 'buat'" x-cloak>
        <div class="max-w-4xl mx-auto">
                            ...
                            <div class="space-y-5">
                                ...
                                <div>
                                    ...
                                    <select name="category_id" class="w-full rounded-xl border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('category_id') border-red-500 @enderror" required>
                                        <option value="" disabled {{ (!isset($draft) && !old('category_id')) ? 'selected' : '' }}>Pilih Kategori</option>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" name="location" placeholder="Gedung A Lantai 2, Ruang 201" 
                                            ...
                                            value="{{ isset($draft) ? $draft->location : old('location') }}" required>
                                    </div>
                                    <p class="text-xs text-gray-400 mt-1">Lokasi kejadian diperlukan untuk penanganan yang tepat</p>
                                    @error('location')
                                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                        ...
                                        <label for="image-upload" 
                                            ...
                                            <div class="flex flex-col items-center justify-center py-6" x-show="!filePreview && !fileName">
                                                ...
                                                <p class="text-xs text-gray-400">
                                                    JPG, PNG, GIF (MAX. 2MB) <span class="text-red-500">*Wajib</span>
                                                </p>
                                            </div>
                                            ...
                                            <input type="file" name="image" id="image-upload" accept="image/*" class="hidden" @change="handleFileSelect($event)" required>
                                        </label>
                                    </div>
            ...
            <div class="mt-4 text-center text-xs text-gray-400 dark:text-gray-500">
                <p>Semua field bertanda <span class="text-red-500">*</span> wajib diisi. Dengan mengirimkan pengaduan, Anda menyetujui ketentuan yang berlaku.</p>
            </div>
        </div>
    </div>
    ...
    {{-- ================= KONTEN TAB HISTORI ================= --}}
    {{-- ================= KONTEN TAB HISTORI (Horizontal Scroll) ================= --}}
<div x-show="activeTab === 'histori'" x-cloak>
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-[0_8px_25px_rgba(37,99,235,0.15)] hover:shadow-[0_12px_35px_rgba(37,99,235,0.25)] transition-all duration-300 border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="p-4" style="overflow-x: auto; white-space: nowrap; scrollbar-width: thin;">
            <div style="display: flex; gap: 1rem; flex-wrap: nowrap;">
                @forelse($complaints as $c)
                    {{-- Card Pengaduan --}}
                    <div class="history-card" style="flex: 0 0 340px; min-width: 340px; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08); transition: all 0.3s ease; white-space: normal; display: inline-block;">
                        
                        {{-- Header Card dengan Status --}}
                        <div style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border-bottom: 1px solid #e5e7eb; padding: 12px;">
                            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                                <h3 style="font-size: 14px; font-weight: 600; color: #1f2937; margin: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 180px;">
                                    {{ $c->judul }}
                                </h3>
                                <span class="px-2 py-0.5 rounded-full text-xs font-semibold
                                    @if($c->status == 'pending') bg-yellow-100 text-yellow-800
                                    @elseif($c->status == 'diproses') bg-blue-100 text-blue-800
                                    @elseif($c->status == 'selesai') bg-green-100 text-green-800
                            ...
                            <div style="display: flex; align-items: center; gap: 12px; font-size: 11px; color: #6b7280;">
                                <span>📂 {{ $c->category->nama_kategori ?? 'Umum' }}</span>
                                <span>📅 {{ \Carbon\Carbon::parse($c->tanggal)->format('d/m/Y') }}</span>
                            </div>
                        </div>
                        
                        {{-- Body Card: Gambar dan Deskripsi --}}
                        <div style="padding: 12px; min-height: 160px;">
                            {{-- Gambar --}}
                            @if($c->image)
                            ...
                            @else
                                <div style="width: 100%; height: 120px; background: #f3f4f6; border-radius: 8px; display: flex; align-items: center; justify-content: center; margin-bottom: 8px;">
                            ...
                            {{-- Deskripsi --}}
                            <p style="font-size: 13px; color: #4b5563; margin: 8px 0 0 0; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; line-height: 1.4;">
                                {{ Str::limit($c->deskripsi, 100) }}
                            </p>
                            
                            {{-- Lokasi --}}
                            @if($c->location)
                                <p style="font-size: 11px; color: #9ca3af; margin-top: 6px; display: flex; align-items: center; gap: 4px;">
                                    <span>📍</span> {{ Str::limit($c->location, 35) }}
                                </p>
                            @endif
                        </div>
                        ...
                        @if($c->feedbacks->count() > 0)
                            <div style="background: #f0fdf4; padding: 10px 12px; border-top: 1px solid #dcfce7; border-bottom: 1px solid #dcfce7;">
                                <div style="display: flex; align-items: start; gap: 6px;">
                                    <span style="font-size: 12px;">💬</span>
                                    <div style="flex: 1;">
                                        <span style="font-size: 10px; font-weight: 600; color: #166534;">Admin:</span>
                                        <p style="font-size: 11px; color: #14532d; margin: 4px 0 0 0; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">
                                            {{ Str::limit($c->feedbacks->last()->pesan, 80) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div style="background: #f9fafb; padding: 12px; border-top: 1px solid #e5e7eb;">
                            ...
                            {{-- Karena route complaint.show tidak ada, kita bisa: --}}
                            {{-- 1. Redirect ke halaman yang sama dengan parameter --}}
                            {{-- 2. Atau menggunakan modal --}}
                            {{-- 3. Atau menampilkan detail di tempat --}}
                            
                            @php
                                // Cek apakah ada route complaint.show
                                $hasShowRoute = Route::has('complaint.show');
                            @endphp
                            
                            @if($hasShowRoute)
                                <a href="{{ route('complaint.show', $c->id) }}" 
                                   style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 8px 12px; background: linear-gradient(135deg, #3B82F6 0%, #6366F1 100%); color: white; font-size: 12px; font-weight: 500; text-decoration: none; border-radius: 8px; transition: all 0.2s;"
                            ...
                            @else
                                {{-- Alternative: Toggle detail dengan Alpine.js --}}
                                <button @click="selectedComplaint = {{ $c->id }}; showDetail = true"
                                        style="display: flex; align-items: center; justify-content: center; gap: 6px; padding: 8px 12px; background: linear-gradient(135deg, #3B82F6 0%, #6366F1 100%); color: white; font-size: 12px; font-weight: 500; border: none; border-radius: 8px; cursor: pointer; width: 100%; transition: all 0.2s;"
                                        onmouseover="this.style.opacity='0.9'"
                                        onmouseout="this.style.opacity='1'">
                                    Detail Pengaduan
                                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @empty
                    <div class="text-center py-12" style="width: 100%;">
                        ...
                        <button onclick="document.querySelector('[x-data]').__x.$data.activeTab = 'buat'" 
                                class="inline-block mt-4 px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-sm font-medium rounded-lg hover:shadow-lg transition-all duration-200">
                            + Buat Pengaduan Sekarang
                        </button>
                    </div>
...
{{-- Modal Detail untuk melihat pengaduan (jika tidak ada route show) --}}
@if(!Route::has('complaint.show'))
<div x-show="showDetail" x-cloak style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000;" 
     x-transition.opacity @click.away="showDetail = false">
    <div style="background: white; border-radius: 16px; max-width: 500px; width: 90%; max-height: 80vh; overflow-y: auto; padding: 20px;">
        <template x-for="complaint in complaints" :key="complaint.id">
            <div x-show="selectedComplaint === complaint.id">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                    <h3 style="font-size: 18px; font-weight: bold;" x-text="complaint.judul"></h3>
                    <button @click="showDetail = false" style="background: none; border: none; font-size: 20px; cursor: pointer;">&times;</button>
                </div>
                <p><strong>Status:</strong> <span x-text="complaint.status"></span></p>
                <p><strong>Kategori:</strong> <span x-text="complaint.category"></span></p>
                <p><strong>Tanggal:</strong> <span x-text="complaint.tanggal"></span></p>
                <p><strong>Lokasi:</strong> <span x-text="complaint.location"></span></p>
                <p><strong>Deskripsi:</strong></p>
                <p x-text="complaint.deskripsi"></p>
                <div style="margin-top: 16px; text-align: right;">
                    <button @click="showDetail = false" style="padding: 8px 16px; background: #3B82F6; color: white; border: none; border-radius: 8px; cursor: pointer;">Tutup</button>
                </div>
            </div>
        </template>
    </div>
