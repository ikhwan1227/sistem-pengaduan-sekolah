...
<div class="space-y-6 px-4 md:px-6 lg:px-8" x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'buat' }" x-init="$watch('activeTab', value => window.location.hash = value)">
    ...
    <div class="border-b border-gray-200 dark:border-gray-700">
        <nav class="flex space-x-4 md:space-x-8" x-data="{ activeTab: window.location.hash ? window.location.hash.substring(1) : 'buat' }" x-init="$watch('activeTab', value => window.location.hash = value)">
            {{-- Tab Buat Pengaduan --}}
...
<script>
    ...
    window.addEventListener('switch-tab', (e) => {
        // Find the Alpine component and update activeTab
        const alpineRoot = document.querySelector('[x-data]');
        if (alpineRoot && alpineRoot.__x) {
            alpineRoot.__x.$data.activeTab = e.detail;
        }
    });
    ...
    if (window.location.hash) {
        ...
        if (['buat', 'histori', 'draft'].includes(tab)) {
            setTimeout(() => {
                const alpineRoot = document.querySelector('[x-data]');
                if (alpineRoot && alpineRoot.__x) {
                    alpineRoot.__x.$data.activeTab = tab;
                }
            }, 100);
        }
    }
