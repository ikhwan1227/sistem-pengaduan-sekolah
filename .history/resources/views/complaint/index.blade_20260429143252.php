    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-600 to-indigo-600">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-white">Histori Pengaduan</h2>
                        <p class="text-xs text-blue-100">Semua pengaduan yang telah Anda kirimkan</p>
                    </div>
                </div>
                <div class="bg-white/20 rounded-full px-3 py-1">
                    <span class="text-sm font-semibold text-white">{{ $complaints->total() }}</span>
                    <span class="text-xs text-white/80"> pengaduan</span>
                </div>
            </div>
        </div>
        ...
        <div class="divide-y divide-gray-100 dark:divide-gray-700">
            @forelse($complaints as $index => $c)
                <div class="p-5 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-all duration-200">
                    {{-- Header Card --}}
                    <div class="flex flex-wrap justify-between items-start gap-3 mb-3">
                        <div class="flex items-center gap-2">
                            <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-xs font-bold text-gray-600 dark:text-gray-400">
                                {{ $index + 1 }}
                            </div>
                        ...
                        <span class="px-2.5 py-1 rounded-full text-xs font-semibold
                            @if($c->status == 'pending') bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400
                            @elseif($c->status == 'diproses') bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400
                            @elseif($c->status == 'selesai') bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400
                    ...
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        @if($c->location)
                            <span class="flex items-center gap-1">
                                ...
                                {{ Str::limit($c->location, 40) }}
                            </span>
                        @endif
                    </div>
                    
                    {{-- Grid: Gambar + Balasan Admin --}}
                            ...
                            @if($c->image)
                                <div class="relative group">
                                    <img src="{{ asset('storage/' . $c->image) }}" 
                                         class="rounded-lg border border-gray-200 dark:border-gray-600 w-full max-w-[140px] h-24 object-cover shadow-sm hover:shadow-md transition-all duration-200"
                                         alt="Gambar pengaduan">
                                    <div class="absolute inset-0 bg-black/40 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 flex items-center justify-center">
                                        <span class="text-white text-xs font-medium">Lihat</span>
                                    </div>
                                </div>
                            @else
                                <div class="w-[140px] h-24 bg-gray-100 dark:bg-gray-700 rounded-lg flex flex-col items-center justify-center border border-gray-200 dark:border-gray-600">
                                    ...
                                    <span class="text-[10px] text-gray-400">Tidak ada gambar</span>
                                </div>
                            @endif
                        </div>
                        
                        {{-- Bagian Balasan Admin --}}
                        <div>
                            @if($c->feedbacks->count() > 0)
                                <div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg p-3 border-l-4 border-green-500 h-24 overflow-y-auto">
                                    <div class="flex items-center gap-2 mb-1">
                                        <svg class="w-4 h-4 text-green-600 dark:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                        </svg>
                                        <p class="text-xs font-semibold text-green-700 dark:text-green-400">Balasan Admin</p>
                                    </div>
                                    <p class="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">
                                        {{ $c->feedbacks->last()->pesan }}
                                    </p>
                                </div>
                            @else
                                <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-3 border border-gray-200 dark:border-gray-600 h-24 flex flex-col items-center justify-center">
                                    <svg class="w-6 h-6 text-gray-400 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                    </svg>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Belum ada balasan</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            @empty
                ...
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2">Belum Ada Pengaduan</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Anda belum membuat pengaduan apapun</p>
                    <button @click="activeTab = 'buat'" 
                            class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white text-sm font-medium rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                        </svg>
                ...
                <div class="p-5">
                    ...
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="px-2 py-1 rounded-full text-xs font-semibold
                            ...
                            @elseif($c->status == 'selesai') bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400
                    ...
                    <div class="text-xs text-gray-500 dark:text-gray-400 mb-1">
                        {{ $c->category->nama_kategori ?? 'Umum' }} | {{ \Carbon\Carbon::parse($c->tanggal)->format('d M Y') }}
                    </div>
                    ...
                    {{-- Grid 2 kolom untuk Gambar dan Pesan Admin --}}
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
    {{-- Kolom Kiri: Gambar --}}
    <div class="flex justify-start">
        @if($c->image)
            <img src="{{ asset('storage/' . $c->image) }}" 
                 class="rounded-lg border border-gray-200 dark:border-gray-600 w-full max-w-[120px] h-20 object-cover"
                 alt="Gambar pengaduan">
        @else
            <div class="w-[120px] h-20 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center border border-gray-200 dark:border-gray-600">
    ...
    <div>
        @if($c->feedbacks->count() > 0)
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 h-20 overflow-y-auto">
                <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Pesan Admin:</p>
                <p class="text-xs text-gray-700 dark:text-gray-300">
                    {{ $c->feedbacks->last()->pesan }}
                </p>
            </div>
        @else
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 h-20 flex items-center justify-center">
                <p class="text-xs text-gray-500 dark:text-gray-400 italic">Belum ada balasan</p>
            </div>
        @endif
    </div>
</div>
